...
if __name__ == '__main__':
    ...
    config=edict()
    config=edict()
    config.upsample_type=args.upsample_type
    ...
    config.midnet_scale=args.midnet_scale
    #TODO 2048 or counted???
    resnet_out_channels=[128,256,512,1024,2048]
    config.midnet_in_channels=resnet_out_channels[args.upsample_layer-1]
    config.midnet_out_channels=512
    config.suffix_out_channels=512
    config.midnet_name=args.midnet_name
    ...
    if args.input_shape==0:
        if args.midnet_name=='psp':
            count_size=max(config.midnet_pool_sizes)*config.midnet_scale
    ...
    if config.dataset.ignore_index == 0:
        config.class_number=len(config.dataset.foreground_class_ids)+1
    else:
        config.class_number=len(config.dataset.foreground_class_ids)

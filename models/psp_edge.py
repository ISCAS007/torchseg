...
class psp_edge(TN.Module):
    ...
    def do_train_or_val(self, args, train_loader=None, val_loader=None):
        ...
        edge_loss_fn = torch.nn.CrossEntropyLoss(
            weight=edge_loss_weight, ignore_index=self.ignore_index)
        ...
        for epoch in range(args.n_epoch):
            edge_weight = edge_base_weight * \
                (1 - epoch/(1.0+args.n_epoch))**edge_power
            for loader, loader_name in zip(loaders, loader_names):
                ...
                writer.add_scalar('%s/edge_loss' %
                                  loader_name, np.mean(edge_losses), epoch)
